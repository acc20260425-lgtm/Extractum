...
} from "../../../sidecars/gemini-browser/src/answer-extractor.js";
...
test("groups split message-content nodes into one assistant answer", async ({ page }) => {
...
test("does not select a broad page container with composer controls", async ({ page }) => {
  await page.setContent(shell(""));
  const baseline = await captureAnswerBaseline(page, "What is happening in football?");
  await page.evaluate(() => {
    document.querySelector("main")?.insertAdjacentHTML(
...
test("uses structural baseline so repeated answer text is not dropped", async ({ page }) => {
  await page.setContent(shell(`
    <section data-turn="assistant" data-response-index="1">
...
test("returns stable after the numeric quiet window rather than hard timeout", async ({ page }) => {
  void page;
  let now = 0;
  const selected = {
    group_id: "response:1",
...
test("returns timeout_latest with debug when candidate never stabilizes", async ({ page }) => {
  void page;
  let now = 0;
  let counter = 0;
...
test("returns missing result instead of null when no answer candidate appears", async ({ page }) => {
  void page;
  let now = 0;
