...
class StrategyTests(unittest.TestCase):
    ...
    def test_chunk_map_reduce_runs_each_chunk_then_merges(self):
        ...
        self.assertIn("research report", client.calls[3][0][1].content)
    ...
    def test_antigravity_chunk_map_reduce_runs_each_chunk_then_separates_reduce_then_merges(self):
        ...
        self.assertEqual(outcome.request_count, 7)
        self.assertEqual(outcome.input_tokens, 70)
        self.assertEqual(outcome.output_tokens, 140)
        self.assertEqual(len(client.calls), 7)
        self.assertIn("Chunk 1 of 2", client.calls[0][0][1].content)
        self.assertIn("Chunk 2 of 2", client.calls[1][0][1].content)
        self.assertIn("highly detailed narrative", client.calls[2][0][0].content)
        self.assertIn("chronologically sorted timeline", client.calls[3][0][0].content)
        self.assertIn("claims and evidence", client.calls[4][0][0].content)
        self.assertIn("action items and open questions", client.calls[5][0][0].content)
        self.assertIn("research report", client.calls[6][0][1].content)
        self.assertIn("Merged summary narrative", client.calls[6][0][1].content)
