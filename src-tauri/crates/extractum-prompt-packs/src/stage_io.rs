    ...
    E: sqlx::Executor<'e, Database = Sqlite>,
{
    ...
    let created_at = extractum_core::time::now_rfc3339_utc();
    sqlx::query(
...
mod tests {
    ...
    use crate::dto::StartYoutubeSummaryRunRequest;
    use crate::seed::seed_builtin_prompt_packs_in_pool;
    use crate::test_schema::prompt_pack_test_pool;
    use crate::youtube_summary::create_youtube_summary_run_skeleton_in_pool;
    use extractum_core::error::{AppError, AppResult};
    ...
    #[tokio::test]
    async fn transcript_analysis_stage_input_serializes_contract_keys_as_snake_case() {
        let (pool, run_id) = test_pool_with_frozen_youtube_summary_run().await;
        let stage = load_transcript_analysis_stage_for_source(&pool, run_id, "source_ref_1")
            .await
            .expect("stage");
        ...
        assert_eq!(value["pack_id"], "youtube_summary");
    ...
    async fn test_pool_with_frozen_youtube_summary_run() -> (sqlx::SqlitePool, i64) {
        ...
        let request = StartYoutubeSummaryRunRequest::new(
            ...
            crate::dto::PromptPackRuntimeProvider::Api,
            None,
            "en".to_string(),
            "standard".to_string(),
            "standard".to_string(),
