...
pub async fn preflight_youtube_summary_run_service_impl(
...
pub use preflight_youtube_summary_run_service_impl as preflight_youtube_summary_run;
...
fn browser_runtime_start_blocking_failure(
    status: &extractum_gemini_browser::GeminiBrowserProviderStatus,
) -> Option<YoutubeSummaryPreflightFailure> {
    if status.status == extractum_gemini_browser::GeminiBrowserProviderStatusKind::Ready {
        return None;
    }
...
pub async fn execute_prepared_api_run(
    ...
    let model_input_limit = resolve_model_input_token_limit(&profile, &effective_model).await;
...
mod tests {
    ...
    use crate::source_port::PromptPackTranscriptSegment;
    use crate::test_schema::prompt_pack_test_pool;
    use crate::youtube_summary::test_support::{
        insert_youtube_video, start_request, ScriptedPromptPackSourceReader,
    };
    use crate::youtube_summary::{
    ...
    use extractum_gemini_browser::{GeminiBrowserProviderStatus, GeminiBrowserProviderStatusKind};
    use extractum_llm::{LlmChatRequest, LlmMessage, LlmRequestError};
    use std::sync::{
    ...
    impl PromptPackBrowserExecutor for RecordingBrowser {
        ...
        ) -> PromptPackBrowserFuture<'_, extractum_gemini_browser::GeminiBrowserRunResult> {
    ...
    async fn prompt_pack_browser_stage_cancelled_while_queued_cancels_browser_job() {
        ...
        extractum_gemini_browser::create_queued_run(
            runs_dir.path(),
            &browser_run_id,
        ...
        let stage_result = run_browser_stage_result_with_cancellation(
            ...
            move || async move {
                ...
                extractum_gemini_browser::finish_run(
                    &runs_root,
                    &browser_run_id_for_hook,
                    cancelled_browser_result(&browser_run_id_for_hook),
                )
                .map_err(|error| extractum_core::error::AppError::internal(error.to_string()))?;
                Ok(())
            },
        );
        ...
        let browser_run = extractum_gemini_browser::list_runs(runs_dir.path(), 10)
            .expect("browser runs")
            .runs
            .into_iter()
        ...
        assert_eq!(
            browser_run.status,
            extractum_gemini_browser::GeminiBrowserRunStatus::Cancelled
        );
    ...
    async fn prompt_pack_browser_stage_cancelled_while_active_stops_sidecar() {
        ...
        .expect("queued browser run");
        extractum_gemini_browser::mark_running(runs_dir.path(), &browser_run_id)
    ...
    fn browser_stage_result_maps_to_prompt_pack_completion_without_tokens() {
        let result = extractum_gemini_browser::GeminiBrowserRunResult {
            run_id: "prompt-pack-42-stage-1001".to_string(),
            status: extractum_gemini_browser::GeminiBrowserRunStatus::Ok,
            text: Some("answer".to_string()),
            message: None,
            manual_action: None,
            artifacts: extractum_gemini_browser::GeminiBrowserArtifactRefs::default(),
            elapsed_ms: 321,
            debug_summary: None,
        };
    ...
    async fn persist_browser_stage_provenance_records_result_identity() {
        ...
        persist_browser_stage_provenance(
            &pool,
            1001,
            &extractum_gemini_browser::GeminiBrowserRunResult {
                ...
                artifacts: extractum_gemini_browser::GeminiBrowserArtifactRefs::default(),
                elapsed_ms: 321,
                debug_summary: Some(extractum_gemini_browser::GeminiBrowserRunDebugSummary {
                    mode: extractum_gemini_browser::GeminiBrowserProviderMode::CdpAttach,
                    composer_found: true,
                    ...
                    answer_completion_reason:
                        extractum_gemini_browser::GeminiBrowserAnswerCompletionReason::Stable,
                    final_text_length: 6,
                    error_stage: None,
                    extraction: None,
                }),
    ...
    async fn delete_prompt_pack_run_rejects_active_runs() {
        ...
        assert_eq!(
            active_error.kind,
            extractum_core::error::AppErrorKind::Conflict
        );
    ...
    fn cancelled_browser_result(run_id: &str) -> extractum_gemini_browser::GeminiBrowserRunResult {
        extractum_gemini_browser::GeminiBrowserRunResult {
            run_id: run_id.to_string(),
            status: extractum_gemini_browser::GeminiBrowserRunStatus::Cancelled,
            text: None,
            message: Some("Cancelled".to_string()),
            manual_action: None,
            artifacts: extractum_gemini_browser::GeminiBrowserArtifactRefs::default(),
