...
async fn execute_synthesis_stage_normalizes_provider_string_readable_items() {
    ...
    let text = extractum_core::compression::decompress_text(&content_zstd).expect("decompress");
...
async fn repaired_synthesis_stage_rejects_unknown_claim_ref_with_quarantine() {
    ...
    let error = crate::json_repair::execute_synthesis_stage_repair_completion(
        &pool,
        stage_run_id,
        completion,
        2,
...
async fn transcript_success_artifacts_roll_back_when_parsed_insert_fails() {
    ...
    crate::stage_io::insert_stage_artifact_in_pool(
        &pool,
        1,
        stage_id,
        "parsed_output",
...
async fn repaired_transcript_analysis_persists_intermediate_entities_for_repair_attempt() {
    ...
    crate::json_repair::execute_transcript_analysis_stage_repair_completion(
        &pool,
        stage_id,
        fake_completion_with_valid_transcript_analysis_json(),
        2,
...
async fn repaired_transcript_success_artifacts_roll_back_when_parsed_insert_fails() {
    ...
    let error = crate::json_repair::execute_transcript_analysis_stage_repair_completion(
        &pool,
        stage_id,
        fake_completion_with_valid_transcript_analysis_json(),
        2,
...
async fn load_stage_artifact_json(
    ...
    let text =
        extractum_core::compression::decompress_text(&content_zstd).expect("decompress artifact");
