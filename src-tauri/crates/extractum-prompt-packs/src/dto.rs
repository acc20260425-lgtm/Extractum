...
mod tests {
    ...
    const PROMPT_PACK_RUN_EVENT: &str = "prompt-pack-run-event";

    #[derive(serde::Serialize)]
    #[serde(rename_all = "camelCase")]
    struct PromptPackRunEvent {
        run_id: i64,
        request_id: String,
        kind: String,
        run_status: String,
    ...
    fn prompt_pack_errors_serialize_exact_json_contract() {
        let errors = [
            extractum_core::error::AppError::validation("client_request_id cannot be empty"),
            extractum_core::error::AppError::not_found("Prompt Pack run 404 not found"),
            extractum_core::error::AppError::conflict("Active Prompt Pack runs cannot be deleted"),
            extractum_core::error::AppError::internal("Database error: connection closed"),
        ];
    ...
    fn start_request_accepts_gemini_browser_runtime_provider() {
        ...
        assert_eq!(
            config.mode,
            extractum_gemini_browser::GeminiBrowserProviderMode::CdpAttach
        );
        assert_eq!(
