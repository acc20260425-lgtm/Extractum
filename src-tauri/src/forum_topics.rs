...
pub(crate) fn legacy_external_id_message_id_expr(item: &str) -> String {
    format!(
        r#"CASE
            WHEN {item}.external_id <> ''
             AND {item}.external_id NOT GLOB '*[^0-9]*'
            THEN CAST({item}.external_id AS INTEGER)
            ELSE NULL
        END"#
    )
}
...
pub(crate) fn resolved_topic_predicate(aliases: &ResolvedTopicAliases<'_>) -> String {
    ...
    let telegram_message = aliases.telegram_message;
    let topic = aliases.topic;
    let matched_topic = aliases.matched_topic;
    let legacy_message_id = legacy_external_id_message_id_expr(item);
 ...
 AND (
        ...
        OR (
            ...
            AND {legacy_message_id} = {topic}.top_message_id
        )
        OR (
            {item}.reply_to_top_id IS NULL
        ...
        OR (
            ...
            AND NOT EXISTS (
                ...
                WHERE {matched_topic}.source_id = {item}.source_id
                  AND (
                        (
                            {telegram_message}.telegram_message_id = {matched_topic}.top_message_id
                        )
                        OR (
                            {telegram_message}.item_id IS NULL
                            AND {legacy_message_id} = {matched_topic}.top_message_id
                        )
...
mod tests {
    ...
    fn resolved_topic_join_prefers_typed_telegram_message_identity() {
        ...
        assert!(
            join.contains("telegram_messages.telegram_message_id = forum_topics.top_message_id")
        );
