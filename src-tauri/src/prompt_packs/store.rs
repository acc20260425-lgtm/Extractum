...
pub(crate) async fn require_prompt_pack_version_id(
    ...
    sqlx::query_scalar("SELECT id FROM prompt_pack_versions WHERE pack_id = ? AND pack_version = ?")
        .bind(pack_id)
        .bind(pack_version)
        .fetch_optional(pool)
        .await
        ...
        .ok_or_else(|| {
            AppError::not_found(format!("Prompt pack {pack_id}@{pack_version} not found"))
        })
}
