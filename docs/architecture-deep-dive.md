...
identifiers. The inline Run Inspector and Run History derive compact operator
views from these run records; history rows do not expose full prompt text,
answer text, artifact paths, or copied diagnostic blobs.

Successful stable runs usually persist the final run record and inline debug
summary only. Failure and non-stable paths may write telemetry, reduced
answer-extraction facts, and full artifacts depending on artifact mode.
