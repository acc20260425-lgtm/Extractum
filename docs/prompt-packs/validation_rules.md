...
### 6.3 `telegram_summary`

| Rule ID | Severity | Layer | Scope | Rule |
|---|---|---|---|---|
| `VR-TS-001` | `error` | `schema` | `pack_data.telegram_summary` | `pack_data.telegram_summary` присутствует при `pack_id = "telegram_summary"` |
| `VR-TS-002` | `error` | `schema` | `source_shape` | `source_shape` входит в enum `channel`, `chat`, `mixed` |
| `VR-TS-003` | `error` | `pipeline` | `sources` | При complete structured result массив `sources` непустой |
| `VR-TS-004` | `error` | `pipeline` | `message_refs` | При complete structured result и usable messages > 0 массив `message_refs` непустой |
| `VR-TS-005` | `error` | `reference` | `message_refs` | `summary_source_id + message_id` уникальны внутри `message_refs`, если `message_id` известен |
| `VR-TS-006` | `error` | `reference` | Telegram message references | Любой `message_ref` или `message_refs[]` внутри `telegram_summary` указывает на существующий `message_refs[].message_ref_id` |
| `VR-TS-007` | `error` | `reference` | Telegram message references | Message ref не должен ссылаться на другой source namespace |
| `VR-TS-008` | `error` | `pipeline` | `key_messages` | При complete `standard`/`deep` result есть хотя бы один key message, если usable messages > 0 |
| `VR-TS-009` | `warning` | `qa` | `importance_score` | Message quality or key message score желательно сопровождается reason fields |
| `VR-TS-010` | `warning` | `qa` | `claims` | Claim со `status_in_corpus = "disputed"` желательно имеет evidence из разных сообщений |
| `VR-TS-011` | `warning` | `qa` | `threads` | Thread summary без `root_message_ref` желательно объяснен в `limitations` |
| `VR-TS-012` | `warning` | `qa` | `forwarded_items` | Forwarded item без origin желательно отражен в `limitations` |
| `VR-TS-013` | `warning` | `qa` | `mixed` | Deep mixed-source result желательно содержит `cross_source_synthesis` |
