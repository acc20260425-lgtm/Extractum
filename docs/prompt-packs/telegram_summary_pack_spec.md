...
- В `standard`, каждый `key_message`, `timeline_event`, `topic`, `thread` и
  `claim` должен иметь непустые `evidence_refs`, `message_ref` или
  `message_refs`.
...
Конкретные сообщения канала, группы или supergroup представлены в
`telegram_summary.message_refs`. Snapshot source types описывают корневой
контекст и период, а message-level identity хранится в pack-local message refs
и связанных evidence/fragment records. Это позволяет поддерживать chats и reply
chains без требования делать каждое chat message отдельным `source_ref`.
...
- `message_refs` обязателен как массив. При complete structured result и
  наличии usable Telegram messages он должен быть непустым.
...
### Объект `TelegramMessageRef`

`telegram_summary.message_refs` — pack-local индекс сообщений, на который
ссылаются `message_ref` и `message_refs` внутри остальных объектов.
...
`history_scope` принимает `current`, `migrated`, `mixed`, `unknown`.

Правила:

- `message_ref_id` уникален внутри `telegram_summary.message_refs`.
- Комбинация `summary_source_id + message_id` должна быть уникальной, если
  `message_id` известен.
- `source_ref_id` указывает на root Telegram source, snapshot или конкретный
  `telegram_post`, если сообщение уже представлено как отдельный source ref.
- `evidence_refs` связывают message ref с конкретными текстовыми фрагментами
  сообщения. Для media-only сообщений evidence допустим только при наличии
  caption, `mediaSummary` или другой текстовой metadata.
- `reply_to_message_ref` и `reply_to_top_message_ref` ссылаются на
  `message_refs[].message_ref_id`, если связанное сообщение присутствует в
  корпусе. Если root сообщения нет в корпусе, поля остаются `null`, а
  ограничение фиксируется в `limitations`.
- `forum_topic_id` и `forum_topic_title` заполняются, если доступны из
  Telegram forum topic metadata.
- `is_migrated_history` и `migration_domain` используются для разделения
  текущей и мигрированной истории. Pack не должен смешивать старый и новый
  namespace без явного `history_scope`.
- Любое поле `message_ref` в других объектах указывает на один
  `message_ref_id`. Любое поле `message_refs` указывает на массив таких ids.
...
- Все значения `message_refs` должны существовать в
...
- `message_ref` должен существовать в `telegram_summary.message_refs`.
...
- Thread должен сохранять reply chain context, а не просто перечислять похожие
  сообщения.
- `root_message_ref` должен существовать в `telegram_summary.message_refs`, если
  root message присутствует в корпусе. Если root отсутствует, отсутствие должно
  быть объяснено в `limitations`.
...
### 4.8 `MessageQualitySignal`

`message_quality_signals` — опциональный, но явно типизированный блок для
оценки полезности сообщений. Он нужен для deep mode и для сценариев поиска
важных сообщений. Поле присутствует всегда как массив, но может быть пустым.

```json
{
  "signal_id": "message_quality_1",
  "message_ref": "msg_ref_2",
  "signal_type": "useful_answer",
  "score": 0.74,
  "reason": "Сообщение отвечает на практический вопрос и закрывает thread.",
  "topic_refs": ["topic_1"],
  "thread_refs": ["thread_1"],
...
- `message_ref` должен существовать в `telegram_summary.message_refs`.
- `score` — best-effort число `0..1`.
- `reason` не должен содержать длинную републикацию исходного сообщения.
- Низкий quality score не должен использоваться для скрытого профилирования
  пользователя; это оценка сообщения в контексте задачи, а не личности автора.

### 4.9 `CrossSourceSynthesis`
...
Pack-level `message_refs` — явный pack-local индекс сообщений в
`telegram_summary.message_refs`. Остальные объекты используют:

- `message_ref`, если нужен один primary anchor;
- `message_refs`, если объект синтезирован из нескольких сообщений.

Эти refs не являются заменой `evidence_refs`: message ref указывает на
сообщение целиком, а evidence ref указывает на проверяемый фрагмент текста или
metadata внутри сообщения.
...
Дополнительная Telegram identity хранится в
`telegram_summary.message_refs[].message_id`, `summary_source_id`,
`history_scope` и, при наличии, в `source_ref.type_data.message_id`.

Правила:

- Нельзя использовать одно только `message_id` без `summary_source_id` и source
  namespace в multi-source runs.
- Любой `message_ref` или `message_refs[]` должен указывать на существующий
  `telegram_summary.message_refs[].message_ref_id`.
...
| `VR-TS-004` | error | `message_refs` | При complete structured result и usable messages > 0 массив непустой |
| `VR-TS-005` | error | `message_refs` | `summary_source_id + message_id` уникальны, если `message_id` известен |
| `VR-TS-006` | error | refs | Любой `message_ref` / `message_refs[]` указывает на существующий `message_ref_id` |
| `VR-TS-007` | error | refs | Message ref не должен ссылаться на другой source namespace |
| `VR-TS-008` | error | `key_messages` | При complete `standard/deep` есть хотя бы один key message, если usable messages > 0 |
| `VR-TS-009` | warning | `importance_score` | Есть score без `importance_reasons` |
| `VR-TS-010` | warning | `claims` | Claim имеет `status_in_corpus = "disputed"` без evidence из разных сообщений |
| `VR-TS-011` | warning | `threads` | Thread summary есть без `root_message_ref` и без объяснения в `limitations` |
| `VR-TS-012` | warning | `forwarded_items` | Forwarded item без origin не отражен в `limitations` |
| `VR-TS-013` | qa | `mixed` | Mixed result без `cross_source_synthesis` в `deep` mode |
...
- `telegram_summary_chat_message_refs` — chat/supergroup messages представлены
  через `message_refs`, хотя root source является `telegram_chat_snapshot`.
...
### OQ-TS-01 — глубина recursive thread summaries
...
### OQ-TS-02 — adaptive scoring weights
