# Public Supergroup Source 22 Takeout Validation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Run a controlled, sanitized public-supergroup Takeout validation for local Telegram `source_id=22`.
...
Validate a completed public supergroup Takeout path after the source `18`
public-channel validation closed the channel and normal-sync comparison rows.
...
- it is the smallest current public/member supergroup candidate with no prior
  Takeout batches;
- it has rich topic, reply, thread, and reaction aggregates;
- it uses account `1`, which completed source `18` Takeout successfully;
- it avoids account `11`, which has recent `TAKEOUT_INIT_DELAY` history;
- it avoids repeating already-partial sources `21` and `110`.
...
- [x] **Step 2: Capture current sanitized source snapshot for source 22**
...
- [x] **Step 3: Capture current Takeout baseline for source 22**

Use durable batch diagnostics for `source_id=22`.
...
git commit -m "docs: record source 22 takeout pre-run"
```
...
```javascript
window.__TAURI__.core.invoke("start_takeout_source_import", { sourceId: 22 })
```
...
```powershell
git diff --check
git add docs/superpowers/plans/2026-05-24-public-supergroup-source-22-takeout-validation.md
git commit -m "docs: mark source 22 live takeout run"
```
...
git diff --check
git add docs/superpowers/plans/2026-05-24-public-supergroup-source-22-takeout-validation.md
git commit -m "docs: mark source 22 post-run capture"
```
...
- a before/after snapshot table for `item_count`,
  ...
  `last_synced_at`;
- a batch summary table for the new source `22` batch using the captured
...
```md
- Source `22` completed public-supergroup Takeout with explicit before/after
  snapshots and row-fidelity evidence.
```

```md
- Source `22` public-supergroup Takeout remained blocked before observations
...
```powershell
rg -n 'Source 22 Public Supergroup Takeout|source `22`|TAKEOUT_INIT_DELAY|Public supergroup Takeout|row-fidelity|duplicate' docs/superpowers/verification/takeout-representative-validation-and-fallback-coverage.md docs/backlog.md
git diff --check
```
...
```powershell
git add docs/superpowers/verification/takeout-representative-validation-and-fallback-coverage.md docs/backlog.md docs/superpowers/plans/2026-05-24-public-supergroup-source-22-takeout-validation.md
git commit -m "docs: record source 22 public supergroup takeout validation"
```
...
```text
- source_id 22 validation outcome;
...
- Spec coverage: The plan covers pre-run watermark proof, live source `22`
