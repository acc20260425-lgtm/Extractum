# Public Supergroup Source 19 Takeout Validation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Run a controlled, sanitized public-supergroup Takeout validation for local Telegram `source_id=19`.
...
Validate a completed public supergroup Takeout path after source `22` produced
only bounded-cancelled partial evidence.
...
- it uses account `1`, which completed source `18` Takeout and produced source
  `22` partial Takeout evidence;
- it avoids immediately retrying source `22`, whose estimate was `125813`;
- it avoids account `11`, which has recent `TAKEOUT_INIT_DELAY` history.

Known limitation:

- source `19` currently has no topic memberships and resolver status
  `never_run`, so this plan should not close forum-topic behavior unless the
  live evidence unexpectedly proves that exact row.
...
- [x] **Step 2: Capture current sanitized source snapshot for source 19**
...
- [x] **Step 3: Capture current Takeout baseline for source 19**

Use durable batch diagnostics for `source_id=19`.
...
```text
content_kind: media_only 28, text_only 1807, text_with_media 115
media_kind: document 1, none 1807, photo 60, sticker 7, video 4, voice 1, webpage 70
...
git commit -m "docs: record source 19 takeout pre-run"
```
...
```javascript
window.__TAURI__.core.invoke("start_takeout_source_import", { sourceId: 19 })
```
...
- a before/after snapshot table for `item_count`,
  ...
  `last_synced_at`;
- a batch summary table for the new source `19` batch using the captured
...
```md
- Source `19` completed public-supergroup Takeout with explicit before/after
  snapshots and row-fidelity evidence.
```

```md
- Source `19` public-supergroup Takeout remained blocked before observations
...
```powershell
rg -n 'Source 19 Public Supergroup Takeout|source `19`|TAKEOUT_INIT_DELAY|Public supergroup Takeout|row-fidelity|duplicate' docs/superpowers/verification/takeout-representative-validation-and-fallback-coverage.md docs/backlog.md
git diff --check
```
...
```powershell
git add docs/superpowers/verification/takeout-representative-validation-and-fallback-coverage.md docs/backlog.md docs/superpowers/plans/2026-05-24-public-supergroup-source-19-takeout-validation.md
git commit -m "docs: record source 19 public supergroup takeout validation"
```
...
- [x] **Step 2: Skip local handoff context update**
...
Previously this plan expected a local handoff update with:

```text
- source_id 19 validation outcome;
...
This step is intentionally satisfied by skipping that local-only update.
...
git diff --check
git add docs/superpowers/plans/2026-05-24-public-supergroup-source-19-takeout-validation.md
git commit -m "docs: complete source 19 public supergroup validation plan"
```
...
- Spec coverage: The plan covers pre-run watermark proof, live source `19`
