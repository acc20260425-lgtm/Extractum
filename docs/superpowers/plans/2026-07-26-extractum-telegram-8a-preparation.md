...
- [ ] Do not edit a migration, schema registry, value registry, frontend source, command registration, IPC signature, event name/payload, status string, error kind/message/JSON, secret identifier, session path, envelope byte/string contract, transaction boundary, cancellation point, message order, limit, or fallback rule.
...
- [ ] The immutable baseline is exactly 140 existing `extractum` test identities. It maps to 140 primaries plus exactly three declared companion identities, for 143 final executable identities. Drift requires a plan amendment and renewed review; it is not repaired during execution.
...
This table is executable plan data. Task 1's TypeScript contract parses it directly and requires exactly 140 unique baseline rows, 99 app primaries, 41 future-crate primaries, and exactly three future-crate companions. `staged_path` is the final 8B app-side portable path even for a value first prepared under `telegram/*.rs` in 8A. App primaries keep their current path unless the row states another final identity.
...
These are plan-added tests, separate from the immutable 140 baseline. The item-kind test is the one mandatory companion already counted in the 143 baseline-plus-companion union; every other row is additional verification.
...
| 5 | `telegram::runtime::tests::authorization_maps_to_ready_and_reauth_required` | crate / `runtime::tests::authorization_maps_to_ready_and_reauth_required` |
...
| 5 | `telegram::runtime::tests::request_code_replaces_pending_attempt` | crate / `runtime::tests::request_code_replaces_pending_attempt` |
...
| 5 | `telegram::runtime::tests::successful_sign_in_returns_session_and_clears_attempt` | crate / `runtime::tests::successful_sign_in_returns_session_and_clears_attempt` |
| 5 | `telegram::runtime::tests::clear_account_aborts_runner_and_ignores_sign_out_failure` | crate / `runtime::tests::clear_account_aborts_runner_and_ignores_sign_out_failure` |
...
The test-only runtime seam is `#[cfg(test)]`, private, and accepts deterministic fake authorization/login/runner behavior. It is not a production transport trait or public runtime abstraction.
...
companions:                    3
baseline + companion union:   143 unique identities
```
...
Before CP3, current executable/source identities equal the 140 baseline. After CP3, the contract maps DTO/media leaves to `telegram::{dto,media}::tests`, preserves all app identities, and admits only the item-kind companion. After CP4 it additionally maps the three codec-subject tests to `telegram::session::tests`. Future 8B/8C mappings are parsed now but remain inactive until those exact paths/manifests exist.
...
Require the Task 0 design/roadmap addendum to be present and contractually encode `src-tauri/src/sources/store.rs` as the sole app-only dependent consumer of `get_client`, not a moved owner. Its 24 app tests are not added to the immutable ownership/move map; Task 5 runs them as a dependent regression suite when the facade call changes. Discovery of any other raw-client/session consumer stops the plan for amendment.
...
Add the two exact Rust identities:
...
# Implement current-value assertions only.
...
Add the minimum test-only pure path helper needed to observe the current temp path without a Tauri runtime, then:
...
# Implement assertions against current helpers.
...
# Implement the exact nine-field shape, then GREEN.
...
# Move the one constant and assertion, then GREEN.
...
Move `TelegramMessageIdentity`, its five fields, three private peer-kind constants, `validate`, and its one baseline test into `telegram/dto.rs`. The moved test's Phase 8A exact identity is:

```powershell
Invoke-ExactRustTest extractum 'telegram::dto::tests::telegram_message_identity_validation_rejects_invalid_values'
```

The six direct `extractum_core::error` paths and all Task 1 message/kind/JSON/precedence assertions must be byte-equivalent apart from module/import movement. Update `ingest_provenance.rs` and `takeout_import/migrated_history.rs` only to consume the curated facade values.
...
# Implement exact URL-safe/no-pad decode and 32-byte validation.
...
# Implement generation without a getter.
...
# Implement legacy format classification returning false.
...
# Implement the exact existing Auth error.
...
For deterministic runtime tests, use a `#[cfg(test)]`-only private fake constructor/callback set. Production code directly owns Grammers; do not add a public or production-generic transport trait.

- [ ] **Step 2: RED/GREEN authorization and missing-account behavior.**

```powershell
Assert-ExactRustRuntimeRed extractum `
  'telegram::runtime::tests::authorization_maps_to_ready_and_reauth_required' `
  'src-tauri/src/telegram/runtime.rs' `
  'RED: CP5 authorization status'
# Implement then GREEN.
Invoke-ExactRustTest extractum 'telegram::runtime::tests::authorization_maps_to_ready_and_reauth_required'
...
```powershell
Assert-ExactRustRuntimeRed extractum 'telegram::runtime::tests::request_code_replaces_pending_attempt' 'src-tauri/src/telegram/runtime.rs' 'RED: CP5 replace login attempt'
Invoke-ExactRustTest extractum 'telegram::runtime::tests::request_code_replaces_pending_attempt'
...
Assert-ExactRustRuntimeRed extractum 'telegram::runtime::tests::successful_sign_in_returns_session_and_clears_attempt' 'src-tauri/src/telegram/runtime.rs' 'RED: CP5 successful sign in'
Invoke-ExactRustTest extractum 'telegram::runtime::tests::successful_sign_in_returns_session_and_clears_attempt'
```

Preserve exact errors `Account not initialized` and `Call tg_send_code first`, the network mapping, phone/token replacement, failed-sign-in retention, successful-sign-in clearing, and returned session.

- [ ] **Step 4: RED/GREEN cleanup and authorized capability behavior.**

```powershell
Assert-ExactRustRuntimeRed extractum 'telegram::runtime::tests::clear_account_aborts_runner_and_ignores_sign_out_failure' 'src-tauri/src/telegram/runtime.rs' 'RED: CP5 runtime clear'
Invoke-ExactRustTest extractum 'telegram::runtime::tests::clear_account_aborts_runner_and_ignores_sign_out_failure'
...
Missing clear remains a no-op; optional sign-out failure remains ignored; runner abort order remains current. Authorized lookup preserves exact missing-account and unauthenticated error strings/kinds.
...
# Implement exact ready/reauth mapping, then GREEN.
...
Clone the current raw client/session at the same ownership points as today; do not hold a state mutex across network awaits. Preserve behavior and run:
...
Start from the clean Checkpoint 5 commit and freeze Task 6 Cargo identity:
...
- authority links, pre-Task-0 starting HEAD, authority-synchronization SHA, Checkpoint 1–5 SHAs, and Checkpoint 5 as the pre-final evidence HEAD;
- checkpoint rollback order and actual changed path inventory;
- 140/99/41/3 identity-map reconciliation plus plan-added tests;
...
If any correctness gate fails, fix only in the owning checkpoint scope, rerun that checkpoint, create a separate narrowly scoped fix commit, and then restart Task 6 from Step 1. Timing never authorizes a fix or rollback.
...
- Revert failed or unwanted checkpoints newest-first with ordinary `git revert <sha>`. Preserve earlier independently useful GREEN checkpoints unless the owner explicitly rejects them.
...
- [ ] The complete diff contains no migration, frontend, manifest, lockfile, dependency, feature, revision, wire-value, command, event, status, or transaction change.
