...
**Architecture:** The two replacement files belong to the existing Vitest `component` project by their `*.behavior.component.test.ts` suffix. Each cohort first proves that its component and route graph can mount under jsdom with bounded API fixtures. Characterization tests then preserve the exact frozen replacement titles: 23 top-level Projects declarations and 20 declarations inside `describe("report canvas component contract")`. Route-owned claims use rendered pages plus API spies; component-owned claims use rendered DOM, callbacks, or direct imports of ordinary TypeScript data. The ledger is changed and each legacy file is deleted only after all replacement declarations for that cohort pass.
...
- Keep the 23 required Projects declarations at top level. Keep the 20 Canvas declarations under the exact `report canvas component contract` describe title.
...
Slice 3B changes no Rust source, Cargo manifest, feature, or cross-crate interface, so it has no focused Rust RED/GREEN loop. The authoritative final `npm.cmd run verify` still executes the unchanged workspace Cargo gates. Use the canonical `src-tauri/target`; do not create another target directory.
...
Inside the existing exact describe, add SC-000151--SC-000154. Render the analysis route with an intentionally narrow `ReportCanvas` receiving-component fixture and interact through its exposed controls to prove:
...
Expected: PASS with all 20 exact declarations under `report canvas component contract` plus two smoke tests.

- [ ] **Step 3: Update replacement paths and delete the legacy file**

Change all SC-000138--SC-000157 replacement paths to `src/lib/analysis-report-canvas.behavior.component.test.ts`, preserving every exact full title and all other ledger metadata. Delete `src/lib/analysis-report-canvas.test.ts`.
...
Expected: PASS with both Slice 3B suites.
...
Run: `git add src/lib/analysis-report-canvas.behavior.component.test.ts testing/source-contract-ledger.json src/lib/analysis-report-canvas.test.ts`
