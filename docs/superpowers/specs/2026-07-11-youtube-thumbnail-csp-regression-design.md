...
4. Rust returns a typed result. Concurrent requests share an in-flight fetch; successful data URLs are memoized only by a bounded frontend LRU.
...
- Validation failures are terminal and may be memoized. Network, timeout, and HTTP-status failures are transient and are not memoized, allowing a later render to retry.
...
- Frontend maps a successful backend result to image `src`; `YoutubeThumbnail` accepts `url` plus an already-local `fallbackSrc` and retains that fallback on error.
