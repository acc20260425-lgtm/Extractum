...
### 2026-05-24 Source 114 Channel Private Takeout Post-Fix Retry

App commit: `7647618`. Working tree was clean before this live retry on branch
`takeout-channel-private-count-probe-fallback-marker`.

Outcome category: `CHANNEL_PRIVATE` fallback.
...
| reaction_item_count | 0 | 0 | 0 |
| last_sync_state | 1 | 1 | unchanged |
| last_synced_at | 1779418693 | 1779647870 | advanced |
...
Warning codes for batch `17`: `only_my_messages_fallback`.

Fallback evidence: present. Batch `17` recorded the
`only_my_messages_fallback` warning code, `only_my_messages = 1`, and
`history_scope = partial_private_history`.
...
Result: batch `17` moves the `CHANNEL_PRIVATE` fallback row from
`needs follow-up` to `passed`. This validates the access-limited fallback and
durable warning/flag path; it does not prove full-history import for this
source because Telegram exposed zero only-my-messages observations.
