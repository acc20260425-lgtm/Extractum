...
function handlerSource(source: string): string {
  const marker = "macro_rules! application_command_inventory", start = source.indexOf(marker);
  if (start < 0) throw new Error("missing application command inventory"); const open = source.indexOf("{", start + marker.length), macro = source.slice(open + 1, matchingDelimiter(source, open, "{", "}"));
