...
function arrangeAnalysisRoute(options: {
  ...
  api.listAnalysisSourceGroups.mockResolvedValue(groups);
...
async function renderAnalysisRouteWithReceiver() {
  analysisRouteReceiverMockActive = true;
  vi.doMock(
    "$lib/components/analysis/report-canvas.svelte",
    analysisRouteReceiverModule,
  );
  // @ts-expect-error The query gives the receiver route its own Vite module identity.
  const analysisPageModule = await import("../routes/analysis/+page.svelte?route-receiver");
  return render(analysisPageModule.default);
}

function releaseAnalysisRouteReceiverMock() {
  if (!analysisRouteReceiverMockActive) {
    return;
  }
  vi.doUnmock("$lib/components/analysis/report-canvas.svelte");
  analysisRouteReceiverMockActive = false;
}
...
afterEach(() => {
  releaseAnalysisRouteReceiverMock();
...
describe("report canvas component contract", () => {
  ...
  it("submits NotebookLM export for either current source or current source group", async () => {
    ...
    const sourceView = await renderAnalysisRouteWithReceiver();
    ...
    await renderAnalysisRouteWithReceiver();
